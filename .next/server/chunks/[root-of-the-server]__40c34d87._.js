    ...
    `)[0]??null;if(!r)return R.NextResponse.json({error:"Offer not found"},{status:404});if(r.driver_id!==t.driver_id)return R.NextResponse.json({error:"Unauthorized"},{status:403});if("pending"!==r.status)return R.NextResponse.json({error:"Offer already responded to",current_status:r.status},{status:409});let a=new Date,i=new Date(r.expires_at);if(a>i&&"accepted"===t.response)return await x`
      ...
      `,await x`
        ...
        SET
          ...
          updated_at = NOW()
        WHERE id = ${s.id}
      `;let e=(0,v.calculateCommissions)(s.total_price,null!==s.source_driver_id);await x`
        UPDATE commissions
        SET
